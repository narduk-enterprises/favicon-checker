 ...
 *
 * Keeps `narduk-nuxt-template` (Level 1 Fleet Hub) in sync with
 * `0_global-canonical-tokens` (Level 0 Canonical Hub).
 *
 * Because Doppler blocks daisy-chaining references, narduk-nuxt-template
 ...
 * into narduk-nuxt-template's dev and prd configs.
 *
 * Usage:
...
console.log(
  DRY_RUN
    ...
    : '🚀 APPLYING changes to narduk-nuxt-template...\n',
)

// Fetch fleet hub's current prd values
const fleetSecrets = getSecrets(FLEET, 'prd')
...
for (const [saasConfig, keyMap] of Object.entries(SYNC_MAP)) {
  ...
  for (const [canonicalKey, fleetKey] of Object.entries(keyMap)) {
    ...
    const fleetValue = fleetSecrets[fleetKey]

    if (!canonicalValue || canonicalValue === 'TO_BE_GENERATED') {
      continue
    }

    if (fleetValue !== canonicalValue) {
      updates[fleetKey] = canonicalValue
      const preview = canonicalValue.slice(0, 12) + '...'
      console.log(`  ${fleetKey} ← ${CANONICAL}.${saasConfig}.${canonicalKey} (${preview})`)
    }
  }
}
