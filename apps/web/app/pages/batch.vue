...
useSeo({
  ...
  description:
    'Check favicons for multiple websites simultaneously. Paste a list of URLs and instantly see all their favicons side by side.',
  ogImage: {
    title: 'Batch Favicon Checker',
    description: 'Check multiple websites at once — free and instant.',
...
useFAQSchema([
  {
    ...
    answer:
      'You can paste as many URLs as you like — one per line, comma-separated, or as a JSON array. Each URL is checked in parallel with concurrency controls to ensure fast, reliable results.',
  },
  {
    question: 'What input formats does the batch checker accept?',
    answer:
      'The batch checker accepts URLs in three formats: one URL per line, comma-separated URLs, or a JSON array of URL strings. We automatically detect the format and parse accordingly.',
  },
  {
    question: 'Are batch results saved?',
    answer:
      "Yes, each individual URL check is saved to our database, so you can revisit results for any domain you've previously checked at its dedicated page.",
  },
])

useScrollReveal()

const { batchInput, batchResults, progress, isProcessing, checkBatch } = useBatchFaviconChecker()
...
const completedUrls = computed(
  () => batchResults.value.filter((r) => r.status === 'done' || r.status === 'error').length,
)
...
<template>
  <div class="min-h-screen">
        ...
        <div class="mb-6 animate-fade-in">
          <NuxtLink to="/" class="group inline-flex items-center gap-3">
            <img
              src="/logo.png"
              alt="Favicon Checker"
              class="size-12 drop-shadow-lg transition-transform group-hover:scale-105"
            />
    ...
    <section class="mx-auto max-w-3xl px-4 pb-12 sm:px-6">
      <div class="card-base animate-slide-up-sm rounded-2xl p-6">
        <div class="mb-4 flex items-center justify-between">
          <h2 class="text-lg font-semibold text-default">URLs to Check</h2>
          <UButton
            variant="ghost"
            size="sm"
        ...
        <div class="mt-4 flex items-center justify-between">
          <p class="text-sm text-dimmed">Newline, comma, or JSON array</p>
          <UButton
      ...
      <div v-if="isProcessing" class="mt-6 animate-fade-in">
        <div class="mb-2 flex items-center justify-between text-sm">
          <span class="text-muted"> Checking {{ completedUrls }} of {{ totalUrls }} sites... </span>
          <span class="font-mono font-medium text-primary-500"> {{ progress }}% </span>
        </div>
    ...
    <!-- Results -->
    <section v-if="batchResults.length > 0" class="mx-auto max-w-4xl px-4 pb-20 sm:px-6">
      <h2 class="mb-6 text-xl font-bold text-default">Results ({{ batchResults.length }})</h2>
      ...
      <div class="space-y-4">
        ...
        >
          ...
          <div class="flex items-center gap-3 border-b border-default px-5 py-3">
            <UIcon :name="getEntryIcon(entry)" :class="getEntryIconClass(entry)" />
            <span class="min-w-0 flex-1 truncate font-mono text-sm text-default">
              {{ entry.url }}
            </span>
            <UBadge v-if="entry.result" color="primary" variant="subtle" size="sm">
              {{ entry.result.favicons.length }} favicon{{
                entry.result.favicons.length === 1 ? '' : 's'
              }}
            </UBadge>
          </div>
          ...
          <div v-else-if="entry.result" class="px-5 py-4">
            <p class="text-sm text-dimmed">No favicons found.</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="mx-auto max-w-3xl px-4 pb-12 text-center sm:px-6">
      <p class="text-muted">Just need to check one site?</p>
    ...
    <section class="mx-auto max-w-3xl px-4 pb-20 sm:px-6">
      ...
      <div class="reveal-on-scroll">
        <UAccordion
          :items="[
            {
              label: 'How many URLs can I check at once?',
              content:
                'You can paste as many URLs as you like — one per line, comma-separated, or as a JSON array. Each URL is checked in parallel with concurrency controls to ensure fast, reliable results.',
            },
            {
              label: 'What input formats does the batch checker accept?',
              content:
                'The batch checker accepts URLs in three formats: one URL per line, comma-separated URLs, or a JSON array of URL strings. We automatically detect the format and parse accordingly.',
            },
            {
              label: 'Are batch results saved?',
              content:
                'Yes, each individual URL check is saved to our database, so you can revisit results for any domain you\'ve previously checked at its dedicated page.',
            },
          ]"
