...
useSeo({
  ...
  description:
    'Upload one image and generate all required favicon sizes instantly. Download ICO, PNG (16×16 to 512×512), Apple Touch Icon, and web manifest — all free, all client-side.',
  ogImage: {
    title: 'Favicon Generator',
    description: 'Upload one image, get every favicon size — free and instant.',
    icon: 'i-lucide-image',
  },
})

useWebPageSchema({
  name: 'Favicon Generator — Create All Sizes from One Image',
  description:
    'Upload one image and generate all required favicon sizes instantly. Download ICO, PNG, Apple Touch Icon, and web manifest.',
})
...
useFAQSchema([
  {
    ...
    answer:
      'Upload any image and our tool resizes it to all required favicon sizes (16×16, 32×32, 48×48, 180×180, 192×192, 512×512) entirely in your browser. Nothing is uploaded to any server.',
  },
  {
    question: 'What image should I upload?',
    answer:
      'Use a square image at least 512×512 pixels for best results. PNG with transparency works great. SVG is ideal since it scales perfectly.',
  },
])
...
async function downloadAll() {
  ...
  for (const fav of generated.value) {
    ...
    const filename =
      fav.size === 180 ? 'apple-touch-icon.png' : `favicon-${fav.size}x${fav.size}.png`
  ...
  <div class="min-h-screen">
    ...
    <section class="mx-auto max-w-3xl px-4 py-12 sm:px-6">
      ...
      >
        ...
        <h2 class="mb-2 text-xl font-semibold text-default">Drop your image here</h2>
        <p class="text-muted">
      ...
      <div v-if="isGenerated" class="space-y-6">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            <img
              :src="sourceDataUrl"
              alt="Source image"
              class="size-16 rounded-xl border border-default object-contain p-1"
            />
            <div>
              <h2 class="text-xl font-bold text-default">{{ generated.length }} Sizes Generated</h2>
              <p class="text-sm text-muted">Ready to download</p>
            </div>
          </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div v-for="fav in generated" :key="fav.size" class="card-interactive rounded-2xl p-5">
            <div
            ...
            <div class="mb-3">
              <p class="font-mono text-lg font-bold text-primary-500">
                {{ fav.size }}×{{ fav.size }}
              </p>
            ...
            <UButton
              ...
              class="press-effect w-full"
              @click="downloadSingle(fav)"
            >
              Download PNG
            </UButton>
          </div>
        ...
        <div class="card-base rounded-2xl p-6">
          <h3 class="mb-3 text-lg font-semibold text-default">
            <UIcon
              name="i-lucide-code"
              class="mr-2 inline-block size-5 align-middle text-primary-500"
            />
            HTML Code
          </h3>
    ...
    <section class="mx-auto max-w-3xl px-4 pb-20 sm:px-6">
      ...
      <div class="reveal-on-scroll">
        <UAccordion
          :items="[
            {
              label: 'Is my image uploaded to a server?',
              content:
            ...
            },
            {
              ...
              content:
                'Use a square image at least 512×512 pixels. PNG with transparency works best. SVG is ideal for scaling.',
            },
            {
              label: 'What is included in the ZIP?',
              content:
                'All PNG sizes (16, 32, 48, 180, 192, 512), a site.webmanifest file, and the HTML code to paste into your head tag.',
            },
          ]"
        />
      </div>
