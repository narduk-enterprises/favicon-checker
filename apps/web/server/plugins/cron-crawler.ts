...
interface CloudflareCronEnv {
  DB?: D1Database
}

export default defineNitroPlugin((nitro) => {
  nitro.hooks.hook('cloudflare:scheduled', ({ env, context }) => {
    context.waitUntil(
      (async () => {
        consola.info('[cron-crawler] Scheduled trigger fired')

        try {
          const d1 = (env as CloudflareCronEnv).DB
          if (!d1) {
          ...
          if (unchecked.length > 0) {
            ...
            consola.info(
              `[cron-crawler] Picked unchecked domain: ${targetDomain} (${unchecked.length} unchecked remaining)`,
            )
          } else {
